###############################################################################################################
# This script takes the DAPG dataset where you can specify the posterior probability threshold and extract
...
pos_data <- read_tsv("/exports/csce/eddie/inf/groups/mamode_prendergast/Data/GTEx_v8_finemapping_DAPG/GTEx_v8_finemapping_DAPG.txt")
pos_data <- pos_data %>% filter(variant_pip>0.1)
pos_data <- pos_data %>% separate(variant_id, c("CHROM", "POS","REF","ALT", "b38"))
pos_data$POS <- as.double(pos_data$POS)

drops <- c("gene_id", "cluster_id", "cluster_pip", "b38")
...
fwrite(x_comp, file="dataset_downsampled_cleaned_caret_5sf_DAPG_0.1_outfile.csv", sep=",")

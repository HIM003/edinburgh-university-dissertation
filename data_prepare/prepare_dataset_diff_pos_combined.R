############################################################################################################################
# This script takes the CaVEMaN, CAVIAR & DAPG dataset where you can specify the posterior probability threshold and extract
# these (i.e. overlap) from the full prepeared dataset.  It also takes the same number of negative samples from the full
# prepared dataset, whilst respecting the number per chromosome
#
############################################################################################################################
...
dapg <- read_tsv("/exports/csce/eddie/inf/groups/mamode_prendergast/Data/GTEx_v8_finemapping_DAPG/GTEx_v8_finemapping_DAPG.txt")
dapg <- dapg %>% filter(variant_pip > PROB_VALUE)
dapg <- dapg %>% separate(variant_id, c("CHROM", "POS","REF","ALT", "b38"))
dapg$POS <- as.double(dapg$POS)
drops <- c("gene_id", "cluster_id", "cluster_pip", "b38","REF","ALT")
dapg <- dapg[ , !(names(dapg) %in% drops)]
dapg$comb <- paste(dapg$CHROM, dapg$POS, dapg$tissue_id, sep="_")
dapg <- dapg %>% distinct(comb, .keep_all= TRUE)
colnames(dapg)[colnames(dapg) == 'tissue_id'] <- "TISSUE"
colnames(dapg)[colnames(dapg) == 'variant_pip'] <- "Probability"
print(head(dapg))
print(dim(dapg))


caveman <- read_tsv("/exports/csce/eddie/inf/groups/mamode_prendergast/Data/GTEx_v8_CaVEMaN_results")
caveman <- caveman %>% filter(Probability > PROB_VALUE)
drops <- c("GENE", "eQTL","REF","ALT")
caveman <- caveman[ , !(names(caveman) %in% drops)]
caveman$comb <- paste(caveman$CHROM, caveman$POS, caveman$TISSUE, sep="_")
caveman <- caveman %>% distinct(comb, .keep_all= TRUE)
print(head(caveman))
print(dim(caveman))



caviar <- read_tsv("/exports/csce/eddie/inf/groups/mamode_prendergast/Data/GTEx_v8_finemapping_CAVIAR/CAVIAR_Results_v8_GTEx_LD_HighConfidentVariants")
caviar <- caviar %>% filter(Probability > PROB_VALUE)
drops <- c("GENE", "eQTL")
caviar <- caviar[ , !(names(caviar) %in% drops)]
caviar$CHROM <- paste("chr", caviar$CHROM, sep="")
caviar$comb <- paste(caviar$CHROM, caviar$POS, caviar$TISSUE, sep="_")
caviar <- caviar %>% distinct(comb, .keep_all= TRUE)
print(head(caviar))
print(dim(caviar))


all <- inner_join(caveman, caviar, by = c("comb" = "comb"))
all <- inner_join(all, dapg, by = c("comb" = "comb"))
print(head(all))
print(dim(all))
print(colnames(all))

drops <- c("TISSUE.x", "TISSUE.y", "POS.x", "POS.y", "CHROM.x", "CHROM.y", "Probability", "Probability.x", "Probability.y")
all <- all[ , !(names(all) %in% drops)]

all$comb <- paste(all$CHROM, all$POS, sep="_")
print(length(unique(all$comb)))


x$comb <- paste(x$chr, x$variant_pos, sep="_")
x <- x %>% distinct(comb, .keep_all= TRUE)

x <- left_join(x, all, by = c("chr" = "CHROM", "variant_pos" = "POS"))

print(dim(x))

x <- x %>% mutate(x, CLASS = ifelse(is.na(TISSUE), "Negative", "Positive"))
print(table(x$CLASS))
...
print(table(x_pos$chr))
print(table(x_neg$chr))

###############################################################################################################
# This script takes the CaVEMaN dataset where you can specify the posterior probability threshold and extract 
...
pos_data <- pos_data %>% filter(Probability>0.1)
...
drops <- c("chr","variant_pos","CLASS","TISSUE", "GENE","eQTL","REF", "ALT", "Probability","comb")
x_comp <- x_comp[ , !(names(x_comp) %in% drops)]
x_comp <- log(x_comp+1)
x_comp <- format(x_comp, digits = 5)
x_comp <- cbind(x_comp, dropped_class)
fwrite(x_comp, file="dataset_downsampled_cleaned_caret_5sf_CaVEMaN_0.1_outfile.csv.gz", sep=",")

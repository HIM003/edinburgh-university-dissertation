...
x <-fread("/exports/eddie/scratch/s1772751/Prepared_Data/index/dataset_downsampled_cleaned.csv.gz", data.table=FALSE)
...
#SuRE data
SuRE <- read_tsv("/exports/csce/eddie/inf/groups/mamode_prendergast/Data/hisham_b38.txt", col_names = FALSE)
colnames(SuRE) <- c("chr", "variant_pos_1","variant_pos", "random")
SuRE <- SuRE %>% separate(random, c("chr_b37", "pos_b37", "SNP_id", "K562", "hepg2"), "_")
drops <- c("variant_pos_1", "chr_b37", "pos_b37", "SNP_id")
SuRE <- SuRE[ , !(names(SuRE) %in% drops)]

#remove duplictaes 
SuRE$comb <- paste(SuRE$chr, SuRE$variant_pos, sep="_")
SuRE <- SuRE %>% distinct(comb, .keep_all= TRUE)
x$comb <- paste(x$chr, x$variant_pos, sep="_")
x <- x %>% distinct(comb, .keep_all= TRUE)


#combine data
SuRE_GTEx <- inner_join(SuRE, x, by=c("comb"="comb","chr"="chr","variant_pos"="variant_pos"))
SuRE_GTEx$K562 <- as.double(SuRE_GTEx$K562)
SuRE_GTEx$hepg2 <- as.double(SuRE_GTEx$hepg2)
#SuRE_GTEx_pos <- SuRE_GTEx %>% filter(K562 <= 0.0001 | hepg2 <= 0.0001)



fwrite(SuRE_GTEx, file="dataset_downsampled_cleaned_SuRE.csv.gz", sep=",")



q()

x <- x %>% mutate(x, CLASS = ifelse(is.na(variant_pip), "Negative", "Positive"))
...
dropped_class <- subset(x_comp, select=c("chr","variant_pos","CLASS","tissue_id"))
drops <- c("chr","variant_pos","CLASS","tissue_id","REF","ALT","variant_pip","comb")
x_comp <- x_comp[ , !(names(x_comp) %in% drops)]
x_comp <- log(x_comp+1)
x_comp <- format(x_comp, digits = 5)
x_comp <- cbind(x_comp, dropped_class)
fwrite(x_comp, file="dataset_downsampled_cleaned_caret_5sf_DAPG_0_9_outfile.csv", sep=",")

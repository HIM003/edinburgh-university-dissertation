...
dataset_2 <- paste("/exports/eddie/scratch/s1772751/Prepared_Data/index/", name_2, sep="")
print(head(dataset))
...
drops <- c("row_number", "chr", "variant_pos", "comb", "TISSUE", "REF", "ALT", "tissue_id")
x <- x[ , !(names(x) %in% drops)]
x <- x %>% filter(CLASS=="Positive")


x2 <-fread(dataset_2, data.table=FALSE)
x2 <- x2[ , !(names(x2) %in% drops)]
x2 <- x2 %>% filter(CLASS=="Negative")
...
#k <- 0.00055
k <- table(x_test$CLASS)[[1]]/(table(x_test$CLASS)[[2]] / k)

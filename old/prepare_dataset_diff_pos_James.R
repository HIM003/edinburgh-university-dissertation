...
pos_data <- read_tsv("/exports/csce/eddie/inf/groups/mamode_prendergast/Data/hisham_b38.txt")
colnames(pos_data) <- c("CHROM", "POS", "POS_1", "mix")
pos_data <- pos_data %>% separate(mix, c("CHROM_b37", "POS_b37","SND_id","p_K562", "p_HepG2"), sep="_")
print(dim(pos_data))
pos_data$p_K562 <- as.double(pos_data$p_K562)
pos_data$p_HepG2 <- as.double(pos_data$p_HepG2)
pos_data <- pos_data %>% filter(p_K562<0.1 | p_HepG2<0.1)
print(dim(pos_data))


pos_data <- pos_data %>% filter(p_K562<0.01 | p_HepG2<0.01)
print(dim(pos_data))

pos_data <- pos_data %>% filter(p_K562<0.001 | p_HepG2<0.001)
print(dim(pos_data))

pos_data <- pos_data %>% filter(p_K562<0.0001 | p_HepG2<0.0001)
print(dim(pos_data))



drops <- c("CHROM_b37","POS_b37", "SND_id", "POS_1")
pos_data <- pos_data[ , !(names(pos_data) %in% drops)]
print(head(pos_data))

x$comb <- paste(x$chr, x$variant_pos, sep="_")
x <- x %>% distinct(comb, .keep_all= TRUE)

x <- inner_join(x, pos_data, by = c("chr" = "CHROM", "variant_pos" = "POS"))

...
# sampling ratios, but explicitly sets the file to use
#
#
#########################################################################################################
...
setwd("/exports/eddie/scratch/s1772751/Prepared_Data/sep_files/files/")
#chr_data <- read_csv(args[1])
chromo <- read_csv("chr_only_.csv")
chr_data <- read_csv("outfile_new_1538.csv.zip")
chr_data <- data.frame(chromo, chr_data)
...
set <- c("chr1","chr2","chr3")
chr_data <- chr_data %>% filter(chr_data$chr %in% set)
...
for (i in c(3)){
  if(i==1) {
    ...
    set <- c(0.015,"Negative")
    labels_ <- c("1.5% Neg Data","100% Neg Data")
    graph_name <- paste(colnames(chr_data)[1],"1_5_Neg","100_Neg",".png",sep = "_")
  } else if (i==4) {
  ...
  print(head(data_full_compare_subset))
 ...
 # g <- ggplot(data_full_compare_subset,aes(x=data_full_compare_subset[[1]], fill=CLASS)) + geom_density(alpha=0.2) + facet_wrap(~chr, ncol = 6, scales = "free") +
  ...
  g <- ggplot(data_full_compare_subset,aes(x=data_full_compare_subset[[2]], fill=CLASS)) + geom_density(alpha=0.2) + facet_wrap(~chr, ncol = 1, scales = "free") +
    ggtitle(colnames(data_full_compare_subset)[1]) + xlab("Genomic Distance") + scale_fill_discrete(labels = labels_)

  g <- g + theme(axis.title.x = element_text(size=50), axis.title.y = element_text(size=50), axis.text.x = element_text( angle = 0,  hjust = 1, size = 24 ), axis.text.y = element_text( size = 24 ),
	 plot.title = element_text(size=40), strip.text = element_text(size=46), legend.text = element_text(color = "blue", size = 64), legend.title = element_text(color = "blue", size = 28) ) + xlim(0,3e5)
  
  #setwd("/exports/eddie/scratch/s1772751/Prepared_Data/sep_files_/graphs")
  ggsave(graph_name, width = 20, height = 30)
